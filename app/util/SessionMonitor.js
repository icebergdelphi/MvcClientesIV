 ...
 * logged out.
 */
Ext.define('MvcClientesIV.util.SessionMonitor', {
  singleton: true,
  ...
  window: Ext.create('Ext.window.Window', {
    ...
    items: [{
      xtype: 'container',
      frame: true,
      html: "Su sesión expira por tener 15 minutos de inactividad. Cualquier trabajo no guardado se perdera automaticamente....Saliendo del sistema </br></br>Si quiere seguir trabajando, haga click en el boton 'Continuar trabajando'.</br></br>"    
    },{
      xtype: 'label',
      text: ''
    }],
    buttons: [{
      ...
      handler: function() {
        Ext.TaskManager.stop(MvcClientesIV.util.SessionMonitor.countDownTask);
        MvcClientesIV.util.SessionMonitor.window.hide();
  ...
  countDown: function() {
    ...
    if (this.remaining < 0) {
      this.window.down('button[#btnSalirSistema]').handler();
    }
  }
